...
describe('Admin - Productos con más ventas - Exportar Excel', () => {
  beforeEach(() => {
    cy.clearCookies();
  ...
  it('Debe permitir exportar archivo Excel desde la página de productos con más ventas', () => {
    ...
    cy.get('span').contains('Productos con más ventas').click();

    // Esperar a que se cargue la página
    cy.wait(5000);

    // Verificar que estamos en la ruta correcta
    cy.url().should('include', '/admin/productos-mas-ventas');
    ...
    cy.task('getDownloadedFiles').then((files) => {
      ...
      // Buscar archivos que coincidan con el patrón de productos más ventas
      const productosFiles = fileList.filter(
        (file) =>
          file.includes('reporte-products') &&
      ...
      if (productosFiles.length === 0) {
        ...
        throw new Error(`No se encontraron archivos con patrón 'reporte-products'. Archivos encontrados: ${fileList.join(', ')}`);
      }
