...
// Pasos: Iniciar sesión como superadmin, navegar a la página de productos, verificar que se encuentre el botón de exportar con el texto "Descargar Excel". Se selecciona el botón y se descarga el archivo. Se verifica que el archivo se haya descargado correctamente.

describe('Admin - Productos - Exportar Excel', () => {
  beforeEach(() => {
    cy.clearCookies();
  ...
  it('Debe permitir exportar archivo Excel desde la página de productos', () => {
    ...
    cy.task('getDownloadedFiles').then((files) => {
      ...
      // Buscar archivos que coincidan con el patrón de productos
      const productosFiles = fileList.filter(
        (file) =>
          file.includes('productos_') &&
          file.endsWith('.xlsx')
      );
      ...
      if (productosFiles.length === 0) {
        ...
        throw new Error(`No se encontraron archivos con patrón 'productos_'. Archivos encontrados: ${fileList.join(', ')}`);
      }

      expect(productosFiles.length).to.be.greaterThan(0);
      return productosFiles[0];
    }).then((fileName) => {
