...
describe('Admin - Transacciones Exitosas - Exportar Excel', () => {
  beforeEach(() => {
    cy.clearCookies();
  ...
  it('Debe permitir exportar archivo Excel desde la página de total de ventas', () => {
    ...
    cy.get('span').contains('Transacciones exitosas').click();

    // Esperar a que se cargue la página
    cy.wait(5000);

    // Verificar que estamos en la ruta correcta
    cy.url().should('include', '/admin/transacciones-exitosas');
